...
cbuffer CBufferPerObject
{
	...
	float4x4 PointLightProjectiveTexture[NUM_POINT_PROJ];
}
...
float4 pixel_shader(VS_OUTPUT IN) : SV_Target
{
    ...
    for (i = 0; i < NUM_LIGHTS && PointLights[i].Enabled; i++)
    {
		...
		float4 ShadowTextureCoordinate = mul(IN.Position, PointLightProjectiveTexture[i]);
		ShadowTextureCoordinate.xyz /= ShadowTextureCoordinate.w;
		float pixelDepth = ShadowTextureCoordinate.z;
		float shadow = ShadowMap.SampleCmpLevelZero(PcfShadowMapSampler, ShadowTextureCoordinate.xy, pixelDepth).x;
