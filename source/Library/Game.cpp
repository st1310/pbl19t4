...
namespace Library
{
	...
	void Game::Draw(const GameTime& gameTime) // Note: Consider storing separate vectors for GameComponent and DrawableGameComponent objects
	{
		for (GameComponent* component : mComponents)
		{
	...
	void Game::InitializeDirectX()
	{
		...
		if (FAILED(hr = D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, createDeviceFlags,
		...
		if (FAILED(hr = direct3DDevice->QueryInterface(__uuidof(ID3D11Device1),
			reinterpret_cast<void**>(&mDirect3DDevice))))
		{
			throw GameException("ID3D11Device::QueryInterface() failed", hr);
		}

		if (FAILED(hr = direct3DDeviceContext->QueryInterface(__uuidof(ID3D11DeviceContext1),
			reinterpret_cast<void**>(&mDirect3DDeviceContext))))
		{
			throw GameException("ID3D11Device::QueryInterface() failed", hr);
		}
