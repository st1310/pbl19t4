...
namespace Library
{
	...
	XMFLOAT3 State::Move()
	{
		if (mCurrentFrameNumber < mFramesCountInState)
		{
			...
			if (NodeList::TryToMoveInNode(mCollider, XMVECTOR({ mCurrentXPosition, height, mCurrentYPosition, 1.0f }), matrixRotation, XMVECTOR({ mCurrentXPosition + mXDistancePerFrame, height, mCurrentYPosition + mYDistancePerFrame, 1.0f }), mCollNode) )
			{
				mCurrentFrameNumber++;
	...
	void State::MoveInit(XMFLOAT2 currentPosition,  std::vector<XMFLOAT2> positions, float mHeight, float rotation, float translationSpeed, float rotationSpeed, Colliders* collider, CollisionNode* collNode)
	{
		mCollider = collider;
		mCollNode = collNode;
