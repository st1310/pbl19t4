...
namespace Library
{
	RTTI_DEFINITIONS(ProxyModel)
		...
		mWorldMatrix(SimpleMath::Matrix::Identity), mScaleMatrix(SimpleMath::Matrix::Identity), mDisplayWireframe(true),
		mPosition(SimpleMath::Vector3::Zero), mDirection(SimpleMath::Vector3::Forward), mUp(SimpleMath::Vector3::Up), mRight(SimpleMath::Vector3::Right)
	{
	...
	void ProxyModel::Update(const GameTime& gameTime)
	{
		...
		worldMatrix.Forward(mDirection);
		worldMatrix.Up(mUp);
		worldMatrix.Right(mRight);
		worldMatrix.Translation(mPosition);

		XMStoreFloat4x4(&mWorldMatrix, worldMatrix); // TODO: mScaleMatrix * worldMatrix
	}
